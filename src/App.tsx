...
import {
  ...
  MessageCircle, Heart, Send, Tag, Users, HelpCircle,
  Wrench, EyeOff, UserPlus
} from 'lucide-react';
...
function CustomerApp() {
  ...
  const [mainTab, setMainTab] = useState<'order' | 'square'>('order');
  ...
  return (
    <div className="app-viewport flex flex-col bg-slate-50 text-slate-800 font-sans overflow-hidden h-[100dvh] relative">
      ...
      {/* Bottom navigation */}
      {identity && (
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-slate-200 flex items-center justify-around py-2 safe-area-bottom shadow-lg lg:hidden">
          <button onClick={() => setMainTab('order')}
            className={`flex flex-col items-center gap-0.5 px-6 py-1 rounded-xl transition-colors ${mainTab === 'order' ? 'text-orange-500' : 'text-slate-400'}`}>
            <ShoppingBag size={20} />
            <span className="text-[10px] font-bold">点单</span>
          </button>
          <button onClick={() => setMainTab('square')}
            className={`flex flex-col items-center gap-0.5 px-6 py-1 rounded-xl transition-colors ${mainTab === 'square' ? 'text-orange-500' : 'text-slate-400'}`}>
...
function SquarePanel({ identity }: { identity: { nickname: string; dorm: string } }) {
  ...
  const loadPosts = useCallback(() => {
    setLoading(true);
    fetchPosts({ type: activeTab, sort }).then(r => setPosts(r.posts as Post[])).catch(() => {}).finally(() => setLoading(false));
  }, [activeTab, sort]);
  ...
  const handleLike = async (postId: string) => {
    try {
      ...
      setPosts(prev => prev.map(p => p.id === postId ? { ...p, likes_count: p.likes_count + (r.liked ? 1 : -1) } : p));
    } catch {}
  };
  ...
  const handleComment = async (postId: string) => {
    ...
    try {
      ...
      setCommentText('');
      // Reload post detail
      const { fetchPostById } = await import('./api');
  ...
  const TabIcon = SQUARE_TABS.find(t => t.key === activeTab)?.icon || HelpCircle;
          ...
          <div className="w-full max-w-md bg-white rounded-t-[28px] sm:rounded-[28px] p-6 max-h-[85vh] overflow-y-auto shadow-2xl space-y-4">
            ...
            <div className="flex items-center gap-4 border-t border-b border-slate-100 py-2">
              <button onClick={() => handleLike(selectedPost.id)}
                ...
                <Heart size={16} /> {selectedPost.likes_count}
              </button>
