...
conn.on('ready', () => {
  ...
  const commands = [
    ...
    `REPO_URL="https://${TOKEN}@github.com/yuwu-git-ai/wolidun.git"`,
    `if [ -d /root/app/.git ]; then cd /root/app && git remote set-url origin "$REPO_URL" && git pull origin master; else cp -r /root/app/data /tmp/app-data-backup 2>/dev/null; rm -rf /root/app && git clone "$REPO_URL" /root/app && cp -r /tmp/app-data-backup /root/app/data 2>/dev/null; fi`,
    `cd /root/app`,
